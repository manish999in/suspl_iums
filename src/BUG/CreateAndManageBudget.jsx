...
const CreateAndManageBudget = () => {
    ...
    const data = {
        dName: "Description",
        dHolder: "Enter Description",
        sName : "Short Deascription",
        sHolder : "Enter Description",
        save: "Save",
        delete: "Delete",
        update: "Update",
        back: "Back",
    ...
    const validateFields = () => {
        ...
        if(!rcds.shortDescription) errors.shortDescription = "Short Description is Required"
        return errors;
    };
    ...
    const handleCreate = async (evt) => {
        ...
        try {
            ...
            if (rcds.budgetId) {
                ...
                const response = await getUpdate("Bug", "ManageBugd", ciphertext,'budget');
                console.log("Update response from backend:", response.data);

                responseData = checkResponseStatus(response,'budget');
            } else {
                // Send data to the backend
                const response = await getSave("Bug", "ManageBugd", ciphertext,'budget');
    ...
    const getData = async () => {
        try {
            ...
            const response = await getList("Bug", "ManageBugd", ciphertext,'budget');
    ...
    const viewRecord = async (budgetId) => {
        deleteCooldown.current = true;

        try {
            setErrorMessages("");
            const ciphertext = encAESData(secretKey, { budgetId });


            const response = await getViewRecord("Bug", "ManageBugd", ciphertext,'budget');
            ...
            if (responseData.rData) {
                ...
                setRcds({
                    budgetId: dataToSet.budgetId,
                    description: dataToSet.description,
                    shortDescription : dataToSet.shortDescription,
    ...
    const handleDelete = async (budgetId) => {
        ...
        try {
            const ciphertext = encAESData(secretKey, { budgetId });
            const response = await getDelete("Bug", "ManageBugd", ciphertext,'budget');
            const responseData = checkResponseStatus(response,'budget');
            ...
            if (responseData.rType === "Y") {
                ...
                setTableData((prevTableData) => {
                    const newData = prevTableData.filter((item) => item.budgetId !== budgetId);
            ...
            <div className="rightArea">
                ...
                <div className="container-body mx-3 mb-2">
                    <form>
                        <div className="row mb-3">
                            <p className="card-title h6">Create and Manage Budget Type</p>
                        </div>
                ...
                <div className="container-body mx-3 mb-3">
                    <p className="h6 card-title list-header">
                        ...
                        <small>
                            {" "}
                            List of Create and Manage Budget Type <span className="parenthesis">(</span>s
...
export default CreateAndManageBudget;
