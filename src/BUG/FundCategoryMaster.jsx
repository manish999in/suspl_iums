...
const FundCategoryMaster = () => {
    ...
    const data = {
        dName: "Fund Category ",
        dHolder: "Enter Fund Category",
        sName: "Remark ",
        sHolder: "Enter Remark",
        save: "Save",
        delete: "Delete",
        update: "Update",
        back: "Back",
    ...
    const validateFields = () => {
        let errors = {};
        if (!rcds.fundCategory) errors.fundCategory = "Fund Category is Required";
    ...
    const handleCreate = async (evt) => {
        ...
        try {
            ...
            if (rcds.fundId) {
                ...
                const response = await getUpdate("Bug", "fundCtg", ciphertext,'budget');
                console.log("Update response from backend:", response.data);

                responseData = checkResponseStatus(response,'budget'); 
            } else {
                // Send data to the backend
                const response = await getSave("Bug", "fundCtg", ciphertext,'budget');
    ...
    const getData = async () => {
        try {
            ...
            const response = await getList("Bug", "fundCtg", ciphertext,'budget');
    ...
    const viewRecord = async (fundId) => {
        deleteCooldown.current = true;

        try {
            setErrorMessages("");
            const ciphertext = encAESData(secretKey, { fundId });


            const response = await getViewRecord("Bug", "fundCtg", ciphertext,'budget');
            ...
            if (responseData.rData) {
                ...
                setRcds({
                    fundId: dataToSet.fundId,
                    fundCategory: dataToSet.fundCategory,
    ...
    const handleDelete = async (fundId) => {
        ...
        try {
            const ciphertext = encAESData(secretKey, { fundId });
            const response = await getDelete("Bug", "fundCtg", ciphertext,'budget');
            const responseData = checkResponseStatus(response,'budget');
            ...
            if (responseData.rType === "Y") {
                ...
                setTableData((prevTableData) => {
                    const newData = prevTableData.filter((item) => item.fundId !== fundId);
    ...
    const filterTableData = () => {
        const filteredData = rowData
            .filter((apiDataRec) => {
                ...
                const matchesCategory = searchRcds.fundCategory
                    ? apiDataRec.fundCategory
                        .toLowerCase()
                        .includes(searchRcds.fundCategory.toLowerCase())
                    : true;

                    const matchesRemarks = searchRcds.remarks
                    ? apiDataRec.remarks
                        .toLowerCase()
                        .includes(searchRcds.remarks.toLowerCase())
                    : true;
                ...
                <div className="container-body mx-3 mb-2">
                    <form>
                        <div className="row mb-3">
                            <p className="card-title h6">Fund Category Master</p>
                        </div>
...
export default FundCategoryMaster;
