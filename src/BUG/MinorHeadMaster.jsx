...
const MinorHeadMaster = () => {
    ...
    const data = {
        dName: "Minor Head Name ",
        dHolder: "Enter Minor Head Name",
        sName: "Minor Head Code ",
        sHolder: "Enter Minor Head Code",
        save: "Save",
        delete: "Delete",
        update: "Update",
        back: "Back",
    ...
    const validateFields = () => {
        let errors = {};
        if (!rcds.minorHead) errors.minorHead = "Minor Head Name is Required";
    ...
    const handleCreate = async (evt) => {
        ...
        try {
            ...
            if (rcds.minorHeadId) {
                ...
                const response = await getUpdate("Bug", "minorHead", ciphertext,"budget");
                ...
                const response = await getSave("Bug", "minorHead", ciphertext,"budget");
    ...
    const getData = async () => {
        try {
            ...
            const response = await getList("Bug", "minorHead", ciphertext, "budget");
    ...
    const viewRecord = async (minorHeadId) => {
        deleteCooldown.current = true;

        try {
            setErrorMessages("");
            const ciphertext = encAESData(secretKey, { minorHeadId });


            const response = await getViewRecord("Bug", "minorHead", ciphertext,"budget");
            ...
            if (responseData.rData) {
                ...
                setRcds({
                    minorHeadId: dataToSet.minorHeadId,
                    minorHead: dataToSet.minorHead,
    ...
    const handleDelete = async (minorHeadId) => {
        deleteCooldown.current = true;
        setShowSave(false);
        setLoading(true);
        ...
        try {
            const ciphertext = encAESData(secretKey, { minorHeadId });
            const response = await getDelete("Bug", "minorHead", ciphertext, "budget");
            const responseData = checkResponseStatus(response,'budget');
            ...
            if (responseData.rType === "Y") {
                ...
                setTableData((prevTableData) => {
                    const newData = prevTableData.filter((item) => item.minorHeadId !== minorHeadId);
            ...
            cellRenderer: (params) => (
                <>
                    <span
                        ...
                        onClick={() => {
                            const id = params.data.minorHeadId;
                            viewRecord(id)
                                ;
    ...
    const filterTableData = () => {
        const filteredData = rowData
            .filter((apiDataRec) => {
                ...
                const matchesCategory = searchRcds.minorHead
                    ? apiDataRec.minorHead
                        .toLowerCase()
                        .includes(searchRcds.minorHead.toLowerCase())
                    : true;

                const matchesRemarks = searchRcds.headCode
                    ? apiDataRec.headCode
                        .toLowerCase()
                        .includes(searchRcds.headCode.toLowerCase())
                    : true;
                return matchesCategory && matchesRemarks;
            })
            .map((item, sno) => ({
                serialNo: sno + 1,
                headCode: item.headCode,
                minorHead: item.minorHead,
                minorHeadId: item.minorHeadId,
            }));
            ...
            <div className="rightArea">
                ...
                <div className="container-body mx-3 mb-2">
                    <form>
                        <div className="row mb-3">
                            <p className="card-title h6">Minor Head Master</p>
                        </div>
                ...
                <div className="container-body mx-3 mb-3">
                    <p className="h6 card-title list-header">
                        ...
                        <small>
                            {" "}
                            List of Fund Category <span className="parenthesis">(</span>s
...
export default MinorHeadMaster;
