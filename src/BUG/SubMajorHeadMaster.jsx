...
const SubMajorHeadMaster=()=>{
  ...
  const data = {
    cname: "Sub Major Head Name",
    dname:"Sub Major Head Code",
    cholder: "Enter Sub Major Head Name",
    dholder:"Enter Sub Major Head Code ",
    headname: "headname",
    headcode:"headcode",
    save: "Save",
    delete: "Delete",
  ...
  const validateFields = () => {
    const errors = {};
    if (!rcds.headname) errors.headname = "Sub Major Head Name is required.";
    if (!rcds.headcode) errors.headcode = "Sub Major Head Code is required.";
    return errors;
  };
  ...
  const handleCreate = async (evt) => {
    ...
    try {
      ...
      if (rcds.mheadId) {
        ...
        // Save case
        const response = await getSave("fin", "subMajorHeadTypeMaster", ciphertext);
        console.log("response in save in subMajorHeadTypeMaster:",response);
  ...
  const viewRecord = async (mheadId) => {
    deleteCooldown.current = true;
    try {
      setErrorMessages("");
      const ciphertext = encAESData(secretKey, { mheadId });

      const response = await getViewRecord("fin", "subMajorHeadTypeMaster", ciphertext,"budget");
      ...
      if (responseData.rData) {
        ...
        setRcds({
          mheadId: dataToSet.mheadId,
          headname: dataToSet.headname,
          headcode:dataToSet.headcode,
...
const handleDelete = async (mheadId) => {
  deleteCooldown.current = true;
  setShowSave(false);
  setLoading(true);
  ...
  try {
    const ciphertext = encAESData(secretKey, {mheadId});
    const response = await getDelete("fin", "subMajorHeadTypeMaster", ciphertext, "budget");
    const responseData = checkResponseStatus(response,'budget');
    if (responseData.rData) {
      const jsonData = JSON.parse(responseData.rData);
      const decryptedData = decAESData(secretKey, jsonData);
    ...
    if (responseData.rType === "Y") {
      ...
      setTimeout(() => {
        ...
        const newData = prevTableData.filter((item) => item.mheadId !== mheadId);
        ...
        cellRenderer: (params) => (
          <>
            <span
              ...
              onClick={() => {
                const id =  params.data.mheadId ;  // Access the row data to get the roleId
...
const filterTableData = () => {
  ...
  const filteredData = rowData
    .filter((apiDataRec) => {
      ...
      const matchesRoleName = searchRcds.headname
        ? apiDataRec.headname.toLowerCase().includes(searchRcds.headname.toLowerCase())
        : true; 
        const matchesAdminRole = searchRcds.headcode
        ? apiDataRec.headcode === searchRcds.headcode
        : true;

        
      // Return true if both conditions are met
      return  matchesRoleName && matchesAdminRole;
    })
    .map((item, sno) =>({
        serialNo: sno + 1,
        headname: item.headname,
        headcode:item.headcode,
        mheadId: item.mheadId,
      }));
  ...
  <div className="container-fluid px-1">
             ...
             <div className="row mb-3">
               ...
               <div className="col-md-6">
                  ...
                  label={data.dname}
                   name={data.headcode}
                   holder={data.dholder}
                   value={rcds[data.headcode]}
                   errorMessage={errorMessages.headcode}
                   onChange={handleChange}
                   icon={icon.default}
                      Maxlength={4}
                    inputMode="numeric"
     ...
     <div className="container-body mx-3 mb-3">
       <p className="h6 card-title list-header">
         ...
         <small>
           {" "}
           List of Sub Major Head<span className="parenthesis">(</span>s
               ...
               <div className="row mb-3">
                 ...
                 <FormText
                  ...
                  label={data.cname}
                   name={data.headname}
                   holder={data.cholder}
                   value={searchRcds[data.headname]}
                 ...
                 <FormText
                  ...
                  label={data.dname}
                   name={data.headcode}
                   holder={data.dholder}
                   value={searchRcds[data.headcode]}
                   onChange={handleSearchChange}
                   icon={icon.default}
                   Maxlength={4}
                   inputMode="numeric"
...
export default SubMajorHeadMaster